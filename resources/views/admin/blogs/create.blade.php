<x-layouts.app :title="__('Create Blog')">
    <div class="max-w-2xl mx-auto py-8">
        <div class="bg-white dark:bg-neutral-900 shadow rounded-xl p-8">
            <h1 class="text-2xl font-bold mb-6 text-accent">Create Blog</h1>
            <form id="blog-form" method="POST" action="{{ route('admin.blogs.store') }}" enctype="multipart/form-data"
                ...
                <div>
                    <label for="excerpt" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Excerpt
                        <span class="text-xs text-gray-500 dark:text-gray-400 ml-1"
                            title="A short summary or introduction for your blog post. Shown in blog listings.">(Brief
                            Description of the Blog)</span>
                    </label>
                ...
                <div class="flex justify-end gap-2">
                    <button type="submit"
                        class="px-6 py-2 bg-accent text-white dark:text-black rounded-md shadow hover:bg-accent-dark transition">Create
                        Blog</button>
                </div>
            </form>
        </div>
